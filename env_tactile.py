...
class IBLSession(gym.Env):
 ...
 #                max_block_mu,
        ...
        self.action_space = spaces.Discrete(2)  # up or down
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(1,))
        self.reward_range = (0, 1)

        # parameters defining environment (session) behavior
# in tactil task, stimulus is not changing over time 
        num_block_mu = 3
        min_block_mu = 0.7
        max_block_mu = 0.9
        #self.num_block_mu = num_block_mu
        #self.min_block_mu = min_block_mu
        #self.max_block_mu = max_block_mu
...
#        self.max_stimulus_strength = max_stimulus_strength
        ...
        self.possible_block_mu = tuple(np.linspace(
            min_block_mu, max_block_mu, num_block_mu))
        self.possible_block_mu_probs = tuple(np.ones(
            num_block_mu) / num_block_mu)
...
#            num_stimulus_strength) / num_stimulus_strength)
    ...
    def step(self,
        ...
        up_action_prob = model_prob_output[0, 0].item()
        down_action_prob = model_prob_output[0, 1].item()
        correct_action = self.trial_sides[self.current_block_within_session][
            self.current_trial_within_block, self.current_rnn_step_within_trial]
        correct_action_index = (1 + correct_action) // 2
        correct_action_prob = up_action_prob if correct_action.item() == -1 else down_action_prob
        ...
        is_blank_rnn_step = up_stimulus == 0 and down_stimulus == 0
        loss = self.loss_fn(
        ...
        timestep_data = dict(
            ...
            up_stimulus=up_stimulus,
            down_stimulus=down_stimulus,
            trial_strength=trial_strength,
            trial_side=trial_side,
            block_side=block_side,
            loss=loss.item(),
            reward=reward.item(),
            up_action_prob=up_action_prob,
            down_action_prob=down_action_prob,
    ...
    def create_stimuli(self):
        ...
        current_block_association_level = np.random.choice([0.1, 0.3, 0.5, 0.7, 0.9])
        ...
        for num_trials in self.num_trials_per_block:
            stimulus_creator_output = create_block_stimuli(
                ...
                block_side_bias_probabilities=current_block_association_level,
# in tactile task, we fix trial strength
        ...
        #return stimuli, trial_strengths, trial_sides, block_sides, current_block_association_level
...
def create_biased_choice_worlds(env_params,
    ...
    by the human; up and down stimuli are presented with different probability
    in blocks of trials."
    """
